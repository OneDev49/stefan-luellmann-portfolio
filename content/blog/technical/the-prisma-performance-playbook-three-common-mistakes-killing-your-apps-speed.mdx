...
tags: [prisma, backend, full-stack]
thumbnail: "x81VdwhEWe9YbBIxpP7fSPQDnVEwZ6Bx1lTY5mqRJHc7iLbp"
...
The <Highlight color="code">include</Highlight> keyword is one of the most commonly abused features in Prisma.
By default, it acts like a <Highlight color="code">SELECT *</Highlight>, fetching every column from the joined table.

If you only need to display the titles of a user's posts, why fetch the <Highlight color="code">content</Highlight>, <Highlight color="code">publishedAt</Highlight>, and <Highlight color="code">tags</Highlight> columns?
This is called **Over-Fetching**.

**The Solution:** Be specific.
Instead of fetching everything, tell Prisma exactly what you need using <Highlight color="code">select</Highlight>.
...
In this code, we use the <Highlight color="code">select</Highlight> keyword to act as a filter.
...
Have you ever run a query inside a loop, or inside a <Highlight color="code">.map()</Highlight> function?
...
Then, the <Highlight color="code">for</Highlight> loop runs.
...
By using <Highlight color="code">select</Highlight> (or <Highlight color="code">include</Highlight>) correctly, you solve both Over-Fetching and the N+1 problem with one uniform solution.
</TextBlock>
...
Since the <Highlight color="code">stats</Highlight> query doesn't depend on the <Highlight color="code">user</Highlight> query, they should run at the same time.
...
By wrapping the promises in <Highlight color="code">Promise.all()</Highlight>, we trigger all three requests simultaneously.
